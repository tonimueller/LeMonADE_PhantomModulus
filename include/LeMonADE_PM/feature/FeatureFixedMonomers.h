...
#ifndef LEMONADE_PM_FEATURE_FEATUREFIXEDMONOMERS_H
#define LEMONADE_PM_FEATURE_FEATUREFIXEDMONOMERS_H
...
public:
  ...
  /**
   * @brief Overloaded for MoveForceEquilibriumBase. See MoveLocalSc and MoveLocalBcc
   *
   * @details Checks if the monomer is able to move and not fixed.
   * Returns \a True if monomer is movable (\a true ) or fixed (\a false ).
   *
   * @param [in] ingredients A reference to the IngredientsType - mainly the system.
   * @param [in] move A reference to MoveForceEquilibriumBase.
   * @return if monomer is movable (true) or fixed (false).
   */
  template<class IngredientsType,class LocalMoveType>
  bool checkMove(const IngredientsType& ingredients, const MoveForceEquilibriumBase<LocalMoveType>& move) const
  {
    //get the number of bond partners of the particle to be moved
    uint32_t monoIndex=move.getIndex();
    const typename IngredientsType::molecules_type& molecules=ingredients.getMolecules();
